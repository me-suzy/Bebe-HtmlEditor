...
<body>
    ...
    <script>
        ...
        // Același titlu+dată ca în categorie: dacă există în index, ștergi instanța(e) vechi(e) și prepui (fără dublură, mereu primul).
        function mergeCategoryBlockIntoIndex(indexHtml, blockHtml) {
            ...
            if (sigNew) {
                const dupBlocks = blocks.filter(function (b) {
                    return articleSignatureFromBlock(b.block) === sigNew;
                });
                if (dupBlocks.length > 0) {
                    const onlyDup = blocks.length === dupBlocks.length;
                    const oneDupFirst = dupBlocks.length === 1 && dupBlocks[0].start === blocks[0].start;
                    const exactSame = dupBlocks.length === 1 && dupBlocks[0].block === blockHtml;
                    if (onlyDup && oneDupFirst && exactSame) {
                        return {
                            ok: true,
                            html: indexHtml,
                            action: 'skip',
                            msg: 'Articolul este deja primul în index (același text ca în categorie).'
                        };
                    }
                    ...
                    return {
                        ...
                        html: ins.html,
                        action: 'move',
                        msg: 'Articolul era și altundeva în listă; l-am pus primul, fără dublură (varianta din fișierul categorie).'
                    };
                }
            }
            const compactNew = compactArticleHtmlWs(blockHtml);
